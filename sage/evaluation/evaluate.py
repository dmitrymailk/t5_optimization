...
def levenstein_dist(
    source,
    correct,
    allow_transpositions=False,
    removal_cost=1.0,
    insertion_cost=1.0,
    replace_cost=1.0,
    transposition_cost=1.0,
):
    table, _ = make_levenstein_table(
        source,
        correct,
        allow_transpositions=allow_transpositions,
        ...
        for j, second_word in enumerate(correct, 1):
            if first_word == second_word:
                ...
                if (
                    ...
                    and min(i, j) >= 2
                    and first_word == correct[j - 2]
                    and second_word == source[i - 2]
                ):
                    table[i][j] = min(
                        table[i][j], table[i - 2][j - 2] + transposition_cost
                    )
                curr_backtraces = []
...
def extract_best_alignment(backtraces):
    ...
    while len(current_path) > 0:
        ...
        while (
            len(last_indexes) > 0
            and last_indexes[-1] == len(neighbor_vertexes_list[-1]) - 1
        ):
            current_path.pop()
            last_indexes.pop()
...
def extract_levenstein_alignments(source, correct, replace_cost=1.0):
    ...
    """
    table, backtraces = make_levenstein_table(
        source, correct, replace_cost=replace_cost
    )
    paths_in_alignments = extract_best_alignment(backtraces)
    basic_alignment_paths = extract_basic_alignment_paths(
        paths_in_alignments, source, correct
    )
...
def get_partition_indexes(first, second):
    ...
    if m <= 1 or n <= 1:
        ...
        levenstein_table, backtraces = make_levenstein_table(
        ...
        for path in best_paths_in_table:
            ...
            for i, j in path[1:]:
                ...
                if i == word_ends[0][first_pos] and j == word_ends[1][second_pos]:
                    if (
                        first_pos > current_indexes[-1][0]
                        and second_pos > current_indexes[-1][1]
                    ):
...
def align_sents(
    source,
    correct,
    return_only_different=False,
    replace_cost=1.0,
    partition_intermediate=True,
    groups_in_source=None,
):
    """
    ...
    if groups_in_source is None:
        groups_in_source = []
    alignments = extract_levenstein_alignments(
        source, correct, replace_cost=replace_cost
    )
    m, n = len(source), len(correct)
    prev = 0, 0
    ...
    for i, j in alignments[0]:
        if i > prev[0] + 1 or j > prev[1] + 1:
            if partition_intermediate:
                partition_indexes = get_partition_indexes(
                    source[prev[0] : i - 1], correct[prev[1] : j - 1]
                )
                if partition_indexes is not None:
                    for pos, (f, s) in enumerate(partition_indexes[:-1]):
                        answer.append(
                            (
                                (prev[0] + f, prev[0] + partition_indexes[pos + 1][0]),
    ...
    if m > prev[0] or n > prev[1]:
        if partition_intermediate:
            partition_indexes = get_partition_indexes(
                source[prev[0] : m], correct[prev[1] : n]
            )
            if partition_indexes is not None:
                for pos, (f, s) in enumerate(partition_indexes[:-1]):
                    answer.append(
    ...
    for pos, (i_ref, j_ref) in enumerate(groups_in_source):
        ...
        while (
            start_in_answer < len(indexes_in_source)
            and indexes_in_source[start_in_answer][0] < i_ref
        ):
            start_in_answer += 1
        ...
        if i == i_ref:
            while (
                end_in_answer < len(indexes_in_source)
                and indexes_in_source[end_in_answer][1] < j_ref
            ):
                end_in_answer += 1
    ...
    for start_in_answer, end_in_answer in positions_in_answer:
        ...
        new_answer.append(
            (
                (answer[start_in_answer][0][0], answer[end_in_answer][0][1]),
...
def make_corrections_data(source_sents, correct_sents, answer_sents):
    ...
    for num, (source, correct, answer) in enumerate(
        zip(source_sents, correct_sents, answer_sents)
    ):
        try:
            pb.update(1)
            correct_indexes = align_sents(
                source, correct, return_only_different=True, replace_cost=1.9
            )
            src_indexes = align_sents(
                source,
                answer,
...
def output_differences(
    ...
    with open(diff_file, "w", encoding="utf8") as fout:
        ...
        for num, sent in enumerate(source_sents):
            ...
            if (
                len(current_false_positives) == 0
                and len(current_false_negatives) == 0
                and len(current_miscorrections) == 0
            ):
                continue
            fout.write(
                "{0}\n{1}\n{2}\n".format(
                    " ".join(sent),
                    " ".join(answer_sents[num]),
                    " ".join(correct_sents[num]),
                )
            )
...
def evaluation(
    ...
    correct_sents = [
        extract_words(line.strip().strip("\ufeff")) for line in corrections
    ]
